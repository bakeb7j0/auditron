    ...
    def run(self, ctx: AuditContext) -> None:
        ...
        try:
            ...
            if res.rc != 0 and not res.out.strip():
                record_error(ctx.db, cid, "run", res.err, res.rc); mark_check(ctx.db, cid, "ERROR", "ps failed"); return
            for line in res.out.splitlines():
                ...
                ctx.db.execute("INSERT INTO processes(host_id,pid,ppid,user,start_time,etime,cmd) VALUES (?,?,?,?,?,?,?)",
