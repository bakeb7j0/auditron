...
class SSHClient:
    def __init__(self, host: dict, timeout: int = 60):
        self.host = host; self.timeout = timeout; self._which_cache = {}
    def which(self, binary: str) -> bool:
        ...
        self._which_cache[binary] = ok; return ok
    def run(self, command: str, check: bool = False, use_sudo: bool | None = None) -> SSHResult:
        ...
        try:
            p = subprocess.run(ssh_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, timeout=self.timeout)
