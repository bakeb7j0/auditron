...
class SSHClient:
    ...
    def which(self, binary: str) -> bool:
        if binary in self._which_cache: return self._which_cache[binary]
    ...
    def run(self, command: str, check: bool = False, use_sudo: bool | None = None) -> SSHResult:
        ...
        sudo = self.host.get("use_sudo") if use_sudo is None else use_sudo
        remote_cmd = command if not sudo else f"sudo -n bash -lc {shlex.quote(command)}"
        ssh_cmd = ["ssh", "-o", "BatchMode=yes", "-p", port]
        if key: ssh_cmd += ["-i", key]
        ssh_cmd += [f"{user}@{ip}", remote_cmd]
        try:
            proc = subprocess.run(ssh_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, timeout=self.timeout)
            rc = proc.returncode
            if check and rc != 0:
                raise RuntimeError(f"SSH command failed rc={rc}: {proc.stderr}")
            return SSHResult(rc, proc.stdout, proc.stderr)
